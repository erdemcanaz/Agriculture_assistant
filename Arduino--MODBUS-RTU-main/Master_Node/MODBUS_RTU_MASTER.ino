...
void master_write_and_read()
{
  ...
  while (true)
  {
    ...
    for (uint8_t i = 1; i< bytes_to_read[frame_mod]; i++){
      B[i] = mySerial.read();
    }
    ...
    for (uint8_t i = frame_mod; i < 3; i++)
    {
      ...
      if (CRC_RESPONSE_RECIEVED == CRC_RESPONSE_EXPECTED)
      {
        ...
        if (bytes_to_read[i] != 5){
          ...
          //
          Serial.println((uint16_t)(B[bytes_to_read[i]-4]*256+B[bytes_to_read[i]-3]));
        }
        return;
      }
    }
